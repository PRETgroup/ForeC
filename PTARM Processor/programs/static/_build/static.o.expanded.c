# 1 "static.c"
# 1 "/Volumes/Data/Documents/Dropbox/PRET/PTARM/programs/static//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "static.c"
...
volatile Task *priorityQueueNonCritical[5] = {0};
volatile Task *priorityQueueMission[5] = {0};
volatile Task *priorityQueueLife[5] = {0};
...
int main(void) {
 ...
 switch (localScheduler.hwThreadId) {
  case (0): // Hardware thread 0.
     ...
     asm volatile("cdp p13, 1, cr0, cr7, cr0, 1"); ptarmUartOutputStr("Static scheduling example.\n\n"); asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
...
eoeHandler: {
 ...
 if (localScheduler.currentTask != 0) {
  ...
  // "Preempted xxx."
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
  ptarmUartOutputStr("  Preempted "); ptarmUartOutputStr(localScheduler.currentTask->name); ptarmUartOutputStr(".\n");
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
  ...
  case (PTARM_SCHEDULER_RELEASE):
   ...
   for (localScheduler.nextStaticTask = localScheduler.staticSchedule; (*localScheduler.nextStaticTask) != 0; localScheduler.nextStaticTask++) {
    ...
    if (currentTask->state == PTARM_TASK_PAUSED && (currentTask->earliestRelease <= localScheduler.basePeriodCounter)) {
     ...
     // "Released xxx."
     asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
     ptarmUartOutputStr("  Releasing "); ptarmUartOutputStr(currentTask->name); ptarmUartOutputStr(".\n");
     asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
...
one: {
 while (foreverTrue) {
  ...
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
  ptarmUartOutputStr("  one pausing.\n");
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
...
two: {
 while (foreverTrue) {
  ...
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
  ptarmUartOutputStr("  two pausing.\n");
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
...
three: {
 while (foreverTrue) {
  ...
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
  ptarmUartOutputStr("  three pausing.\n");
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
...
four: {
 while (foreverTrue) {
  ...
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 1");
  ptarmUartOutputStr("  four pausing.\n");
  asm volatile("cdp p13, 1, cr0, cr7, cr0, 0");
