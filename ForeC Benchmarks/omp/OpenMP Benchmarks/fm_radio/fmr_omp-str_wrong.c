...
const int KSMPS = 1024;
const int FM_MAX = 5;
const int FM_LIMIT = (65536/2) - 1;
const int GAIN_LMR = 2000;
...
int read_opts(int argc, char* argv[], FILE** input_file, FILE** output_file, FILE** text_file) {	
...
int main(int argc, char* argv[]) {
	...
	if (read_opts(argc, argv, &input_file, &output_file, &text_file) == 1) {
		return 1;
    }
...
//#pragma omp parallel default(shared) private(read_buffer, fm_qd_value, band_11, band_12, band_21, band_22, resume_1, resume_2, ffd_value)
	//{
		while (fread(read_buffer, sizeof(float), 16, input_file) == 16) {
			int i;

			//#pragma omp parallel for default(shared) private(i, fm_qd_value, band_11, band_12, band_21, band_22, resume_1, resume_2, ffd_value)
//#pragma omp parallel default(shared) private(fm_qd_value, band_11, band_12, band_21, band_22, resume_1, resume_2, ffd_value)
	//{
#pragma omp parallel for default(shared) private(i) firstprivate(read_buffer, fm_qd_value, band_11, band_12, band_21, band_22, resume_1, resume_2, ffd_value)
			for (i = 0; i < 8; i++) {
