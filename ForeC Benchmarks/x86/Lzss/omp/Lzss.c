    ...
    if (argc>1 && !strcmp(argv[1], "-h"))
    {
		...
		for (count = 3; count < argc; count++) // Filelist start from the 4th argument.
		{
			...
			else
			{
                ...
                // Cannot parallelise because the file content has to be written sequentially
...
void longestMatch(const Fifo win, const Fifo lk, int begin)
{
    ...
    int i, j, k, _offset;
    #pragma omp parallel for shared(begin, matches_thread, offset_thread) private(i, j, k, _offset, _matches)
    for (i = begin; i < win.written_size; i += NUM_THREADS)
    {
        _matches = 0;
