...
void arctan(
	...
	if (init == 0) {
		...
		*angle_out = 0;
		...
		unsigned int ExactInternal = 1048575 << 12;		//((unsigned int) pow(2.0, max_iq_width_c) - 1) << (32-max_iq_width_c);

		unsigned int LSB_Ang    = 1 << (32 - max_angle_width_c);
		unsigned int LSB_Int    = 1 << (32 - max_iq_width_c);
		unsigned int LSB_OutAng = 1 << (32 - coriq0_alpha_width_c);
 
		/* INPUT PATTERN */
		long xin = data_in_real;
		long yin = data_in_imag;
		...
		signed int x_i = x_i_temp0 + x_i_temp1;	// abs(xin << (30 - coriq0_in_width_c)) + abs(yin << (30 - coriq0_in_width_c));
		...
		signed int y_i = y_i_temp0 - y_i_temp1;	// abs(yin << (30 - coriq0_in_width_c)) - abs(xin << (30 - coriq0_in_width_c));
		
		signed int z_i = 512 << 20;	// ((int) pow(2.0, max_angle_width_c-3)) << (32 - max_angle_width_c);
		...
		for (i = 1; i < cordic_steps_c; i++) /*#11#11*/ {
			if (y_i > 0) {
				x_i_1 = x_i + (((y_i / ((int) pow(2.0, i+i/2))) & ExactInternal)   + ((y_i / ((int) pow(2.0, i+i/2-1))) & LSB_Int));
				y_i_1 = y_i - (((x_i / ((int) pow(2.0, i-i/2))) & ExactInternal)   + ((x_i / ((int) pow(2.0, i-i/2-1))) & LSB_Int));
 		...
 		long angle  = (z_i + ((z_i << 1) & LSB_OutAng)) / ((int) pow(2.0, 32 - coriq0_alpha_width_c));
 
		if (xin >= 0 && yin < 0) {
			angle = -(angle);
		} else if (xin < 0 && yin >= 0) {
			angle = (INTMAX / ((int) pow(2.0, 32 - coriq0_alpha_width_c))) + 1 - angle;
		} else if (xin < 0 && yin < 0) {
			angle = angle - (INTMAX / ((int) pow(2.0, 32-coriq0_alpha_width_c))) - 1;
		} else if (xin == 0 && yin == 0) {
			angle = -1;
 		}
