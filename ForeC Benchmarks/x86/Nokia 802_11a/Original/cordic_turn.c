...
void cordic_turn(
	...
	if (init == 0) {
		...
		// CALCULATION
		int msb_reg = (int)(angle_in_reg / ((int) pow(2.0, AngLen - 3))) & 7;
  
  		long z_reg;
		if(((angle_in_reg / ((int) pow(2.0, AngLen - 3))) & 1) == 1) {
			z_reg = -1;
		} else {
			z_reg = 0;
		}
		...
		for(i = 1; i < Stages; i++) /*#12#12*/ {
			if(z_reg < 0){
				x_temp = x_reg + (y_reg / ((int) pow(2.0, i))) + ((y_reg / ((int) pow(2.0, i-1))) & 1);
				y_temp = y_reg - (x_reg / ((int) pow(2.0, i))) - ((x_reg / ((int) pow(2.0, i-1))) & 1);
				z_reg  = z_reg + (arc_tan_array_c[i-1] / ((int) pow(2.0, 34 - IntAngLen))) + ((arc_tan_array_c[i-1] / ((int) pow(2.0, 33 - IntAngLen))) & 1);
			} else {
				x_temp = x_reg - (y_reg / ((int) pow(2.0, i))) - ((y_reg / ((int) pow(2.0, i-1))) & 1);
				y_temp = y_reg + (x_reg / ((int) pow(2.0, i))) + ((x_reg / ((int) pow(2.0, i-1))) & 1);
				z_reg  = z_reg - (arc_tan_array_c[i-1] / ((int) pow(2.0, 34 - IntAngLen))) - ((arc_tan_array_c[i-1] / ((int) pow(2.0, 33 - IntAngLen))) & 1);
		...
		float res_y;
		if (Latency_on == 0) {
			res_x = (float) (x_reg / ((int) pow(2.0, IntXYLen - OutLen + 2))) / (float)1; // pow(2.0, (double) InLen_frac_bits);
			res_y = (float) (y_reg / ((int) pow(2.0, IntXYLen - OutLen + 2))) / (float)1; // pow(2.0, (double) InLen_frac_bits);
		} else {
			res_x = (float) (res_x_latency[0] / ((int) pow(2.0, IntXYLen - OutLen + 2))) / (float)1; // pow(2.0, (double) InLen_frac_bits);
			res_y = (float) (res_y_latency[0] / ((int) pow(2.0, IntXYLen - OutLen + 2))) / (float)1; // pow(2.0, (double) InLen_frac_bits);
		}
		...
		if (FS_on == 1) {
			*res_output_x = (res_out_x * fs_out_scale) / 16;
			*res_output_y = (res_out_y * fs_out_scale) / 16;
		} else {
			*res_output_x = xin;
			*res_output_y = yin;
		}
