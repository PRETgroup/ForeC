...
void ntaps_filter_ffd(ntaps_filter_conf* conf, const int input_size, const float input[], float* result) {
	...
	for (i = 0; i < conf->taps; i++) {
		sum += conf->history[(conf->next + i) % conf->taps] * conf->coeff[conf->taps - i - 1];
    }
...
int main(int argc, char* argv[]) {
	...
	while (fread(read_buffer, sizeof(float), 16, input_file) == 16) {
		...
		//fprintf(text_file, "fm_qd_value = %f\t band_11 = %f\t band_12 = %f\t resume_1 = %f\t resume_2 = %f\t ffd_value = %f \n", fm_qd_value, band_11, band_12, resume_1, resume_2, ffd_value);
	}
