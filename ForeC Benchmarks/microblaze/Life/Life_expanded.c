...
mainParCore0: {
 ...
 // forec:scheduler:counter:start
 do { asm volatile ("get\t%0,rfsl" "2" : "=d" (counter.current)); counter.current -= 3; } while (counter.current < 0);
 // Stop counting the elapsed cycles for the current reaction.
 asm volatile ("put\t%0,rfsl" "2" :: "d" (0));
 asm volatile ("get\t%0,rfsl" "2" : "=d" (counter.current)); counter.current -= 3;
 counter.difference = counter.current - counter.last;
...
do_stepParHandlerMaster0: {
 ...
 if (do_stepParParent.parStatus == FOREC_PAR_OFF) {
  // Release the mutex.
  asm volatile ("put\t%0,rfsl" "0" :: "d" (((30) << 1) + 0));
 ...
 if (parId == 1) {
  ...
  asm volatile ("put\t%0,rfsl" "0" :: "d" ((30 << 1) + 1));
