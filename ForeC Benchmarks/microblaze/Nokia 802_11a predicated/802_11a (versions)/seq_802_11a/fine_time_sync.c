...
void fine_time_sync(int init, int burst_start_found, int data_in_real, int data_in_imag, int * data_out_real, int * data_out_imag, int *  maximum_found_out) {
	...
	if (init == 0) {
		...
		if (downsampling_flag == 0) {
			...
			time_corr_long_norm = time_corr_long_re_temp*time_corr_long_re_temp + time_corr_long_im_temp*time_corr_long_im_temp;	// ((abs(time_corr_long_re)>>2) * (abs(time_corr_long_re)>>2) + (abs(time_corr_long_im)>>2)*(abs(time_corr_long_im)>>2));
