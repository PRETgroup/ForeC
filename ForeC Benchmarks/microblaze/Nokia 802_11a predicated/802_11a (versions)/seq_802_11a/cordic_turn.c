...
//	int InLen_frac_bits = 0; // number of fractional bits in InLen
	...
	// Initialization
	if (init == 0) {
		for (i = 0; i < ((Stages/IntRegStages) + 1); i++) /*#5#5*/ {
			res_x_latency[i] = 0;
			res_y_latency[i] = 0;
		}
	} else {
		// INPUT PATTERN 
		x_in_reg = (long)(xin * 1);	// (long)(xin * pow(2.0, (double) InLen_frac_bits));
		y_in_reg = (long)(yin * 1);	// (long)(yin * pow(2.0, (double) InLen_frac_bits));
