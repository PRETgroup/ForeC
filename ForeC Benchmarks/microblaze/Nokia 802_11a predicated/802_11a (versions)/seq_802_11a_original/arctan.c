...
#define max_angle_width_c     12
  ...
  int alpha_out_latency[(cordic_steps_c/inter_reg_steps_c)+2];
  unsigned int ExactInternal, ExactAngle;
  unsigned int LSB_Ang, LSB_OutAng, LSB_Int, LSB_OutInt;
  ...
  if (init == 0){
    ...
    rad_tmp = 0; 
    *angle_out = 0;
    for (i = 0; i < ((cordic_steps_c/inter_reg_steps_c) + 1); i++)
      {
 ...
 LSB_OutInt = 1 << (32 - coriq0_out_width_c);
 ...
 /* CALCULATION */
 if (xin == -pow(2, coriq0_in_width_c-1)) xin++;
 if (yin == -pow(2, coriq0_in_width_c-1)) yin++;
 ...
 z_i = ((int) pow(2.0, max_angle_width_c-3)) << (32 - max_angle_width_c);
 ...
 if (angle == pow(2, coriq0_alpha_width_c-1))
   angle = -angle;
 
 /* OUTPUT PATTER */
 if(Latency_on == 0)
   { alpha_out = (int)(angle);}
 else { alpha_out = (int)(alpha_out_latency[0]);}
 ...
 if (oversampling_factor == 1) {
   *angle_out = act_angle >> 5;
 } else if (oversampling_factor == 2) {
   *angle_out = act_angle >> 6;
 } else if (oversampling_factor == 4) {
   *angle_out = act_angle >> 7;
 }
 
 for(i=1;i<(cordic_steps_c/inter_reg_steps_c) + 2;i++)
   { alpha_out_latency[i-1] = alpha_out_latency[i]; }
 if(cordic_steps_c%inter_reg_steps_c == 0)
   { alpha_out_latency[(cordic_steps_c/inter_reg_steps_c)] = angle;}
 else
   { alpha_out_latency[(cordic_steps_c/inter_reg_steps_c) + 1] = angle;}
  }
}
