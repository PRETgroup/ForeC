  ...
  for (k = 0; k < input_size; ++k)
    {
      ...
      //printf ("%-10.5f \t %-10.5f\n", freq_sync_correl_2STS_input.re, freq_sync_correl_2STS_input.im);
      // Autocorrelation of two STS for burst detection
      //fprintf (input, "%s \t %s\n", re_in, im_in);
      //fprintf (input, "%d \t %d\n", (int)(freq_sync_correl_2STS_input.re), (int)(freq_sync_correl_2STS_input.im));
      ...
      //fprintf (output, "%d \t %d\n", (int)(freq_sync_correl_2STS_input.re), (int)(freq_sync_correl_2STS_input.im));
      //fprintf (output, "%d \t %d \t %d \t %d\n", correlation_1ST.re, correlation_1ST.im, freq_sync_correl_2STS_output.re, freq_sync_correl_2STS_output.im);
      ...
      //fprintf (input, "%d \t %d \t %d \t %d\n", init_modules, rx_start, correlation_1ST.re, correlation_1ST.im);
      ...
      //fprintf (output, "%d \t %d \t %d \t %d \t %d \t %d \t %d\n", burst_found_out, burst_found_pulse_out, burst_found_pulse_for_FT, freq_est_phase_out.re, freq_est_phase_out.im, freq_est_norm_out, freq_est_norm_max_out);
	...
	if (maximum_found == 1)
	  {
	    //mintaka__enable= counter > 41000 && counter < 41050; counter ++;
