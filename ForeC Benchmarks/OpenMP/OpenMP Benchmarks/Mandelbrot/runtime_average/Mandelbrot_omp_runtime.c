...
unsigned mandel_double(double cr, double ci) {
	...
	for (i = 0; i < MAX_ITER; i++)
	{
		...
		zi *= 2 * zr;
		zi += ci;
		zr = tmp;		//Cannot parallelize due to the sequential nature of updating zr for the next iteration.
...
void display_double(int size, double xmin, double xmax, double ymin, double ymax) {
	...
	unsigned counts;
	#pragma omp parallel for shared(xscal, yscal, xmin, ymin) private(y, x, cr, ci, counts)
	for (y = 0; y < size; y++) {
		for (x = 0; x < size; x++) {
			cr = xmin + x * xscal;
			ci = ymin + y * yscal;
